          ...
          rows.map((fix) => {
            ...
            return (
              ...
              >
                ...
                <span className="num text-right text-[10.5px] text-[var(--text-3)]">
                  {port.location
                    ? `${distanceNm(fix, port.location).toFixed(0)}nm`
                    : "—"}
                </span>
                <span
                  className={cn(
                ...
                <span className="truncate text-[10px] text-[var(--text-2)]">
                  {slot
                    ? slot.waitHours < 1
                      ? "Proceed to berth"
                      : slot.recommendedSpeedKn != null &&
                          fix.sogKn - slot.recommendedSpeedKn > 0.3
...
export function ArrivalSequence({
  ...
  if (!slots.length) {
    return (
      <EmptyNote>No arrivals predicted inside the current horizon.</EmptyNote>
    );
  }

  const first = slots[0].etaMs;
  const last = Math.max(
    ...slots.map((slot) => Math.max(slot.etaMs, slot.alongsideMs)),
  );
    ...
    <div className="px-2 pb-2">
      <div className="mb-1.5 flex items-baseline justify-between gap-2 text-[10px] text-[var(--text-3)]">
        <span>
          ...
          · <span className="num text-[var(--text-2)]">{plan.occupied}</span>{" "}
          occupied
        </span>
        <span>
      ...
      <ul className="space-y-[3px]">
        {slots.map((slot) => {
          ...
          const width = Math.max(
            2,
            ((slot.alongsideMs - slot.etaMs) / span) * 100,
          );
          return (
            <li key={slot.fix.id}>
              <button
...
export function PortWeatherPanel({
  ...
  const readings = useMemo(() => {
    ...
    return steps.map((hours) => {
      ...
      const station = frame.stations.find(
        (entry) => entry.portCode === port.code,
      );
      const rain =
        station?.rainMm ??
        sampleField(frame, "precipitation").sample(
          port.location!.lon,
          port.location!.lat,
        ).value;
  ...
  if (!timeline.available) {
    return (
      <EmptyNote>
        No weather artefact in this run; the forecast panel is unavailable.
      </EmptyNote>
    );
  }

  const peak = readings.reduce<{ hours: number; impact: number } | null>(
    (worst, row) => {
      if (row.impact == null) return worst;
      if (!worst || row.impact > worst.impact)
        return { hours: row.hours, impact: row.impact };
      return worst;
    },
    null,
  );
  ...
  const rising =
    peak != null &&
    readings[0]?.impact != null &&
    peak.impact > readings[0].impact * 1.15;
        ...
        {(
          [
            [
              "Anchorage pressure",
              rising,
              "More vessels hold offshore as the impact index climbs.",
            ],
            [
              "Arrival window risk",
              rising,
              "Arrival windows tighten when the impact index peaks.",
            ],
            [
              "Turnaround risk",
              (peak?.impact ?? 0) >= 0.18,
...
export function PortSummary({
  ...
  return (
    <>
      <PanelSection
        title="Traffic now"
        right={`${traffic.all.length} in the approach`}
      >
      ...
      <PanelSection title="Port state" right={port.regime}>
        ...
        <Field label="80% band">
          <span className="num">
            {port.forecastQ10?.toFixed(0) ?? "—"}–
            {port.forecastQ90?.toFixed(0) ?? "—"}
          </span>
        </Field>
      ...
      {plan && plan.slots.length ? (
        <PanelSection
          title="Next arrivals"
          right={`${plan.slots.length} in 48h`}
        >
                ...
                >
                  ...
                  <span className="min-w-0 truncate text-[10.5px] text-[var(--text)]">
                    <span
                      ...
                      style={{
                        background: VESSEL_CLASSES[slot.fix.vesselClass].color,
                      }}
                    />
                    {slot.fix.name}
                  </span>
        ...
        <PanelSection title="Decision queue" right="unavailable">
          <p className="text-[10.5px] leading-snug text-[var(--text-3)]">
            <span className="text-[var(--crit)]">
              Artefact not in this run.
            </span>{" "}
            The pipeline exported no decision for {port.name}, so there is
            nothing to act on here — not an empty queue, a missing one.
          </p>
        </PanelSection>
      ) : null}
