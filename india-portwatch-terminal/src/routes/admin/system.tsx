...
import { FreshnessPanel } from "@/components/fabric/FreshnessPanel";
...
function SystemState() {
  ...
  if (health.isLoading)
    return <LoadingPanel label="Probing the service" rows={8} />;
  if (health.isError) {
    return (
      <Page>
    ...
    <Page>
      <PageHeader
        ...
        meta={
          <>
            ...
            <span className="num">
              server {formatUtc(data?.serverTimeUtc ?? null)}
            </span>
          </>
        }
      />
      ...
      <StatStrip
        ...
        items={[
          ...
          {
            label: "Licence mode",
            value: data?.licenceMode?.mode ?? "unknown",
            tone: data?.licenceMode?.stated ? "ok" : "warn",
            note: data?.licenceMode?.stated
              ? `stated in the ${data.licenceMode.source}`
              : "not stated; the most restrictive mode applies",
          },
          {
            label: "Auth mode",
            value: mode,
            ...
            note:
              mode === "demo"
                ? "published credentials, no security"
                : "identity provider",
          },
        ]}
      />
          ...
          <div className="px-3 py-2">
            ...
            <KeyValue label="Forecast origin" dense>
              <span className="num text-[11.5px]">
                {formatUtc(data?.forecastOrigin ?? null)}
              </span>
            </KeyValue>
            ...
            <KeyValue label="Out-of-fold rows" dense>
              <Num
                value={benchmark.data?.summary?.testRows ?? null}
                digits={0}
              />
            </KeyValue>
          </div>
        ...
        <Panel title="Artefacts">
          <div className="px-3 py-2">
            {artefacts.map(([name, present]) => (
              ...
              >
                <span className="num truncate text-[11.5px] text-[var(--text-2)]">
                  {name}
                </span>
                <Pill tone={present ? "ok" : "crit"}>
          ...
          <Panel title="Access">
            <div className="px-3 py-2">
              ...
              <KeyValue label="Session issued" dense>
                <span className="num text-[11.5px]">
                  {formatUtc(session?.issuedAt ?? null)}
                </span>
              </KeyValue>
              <KeyValue label="Session expires" dense>
                <span className="num text-[11.5px]">
                  {formatUtc(session?.expiresAt ?? null)}
                </span>
              </KeyValue>
              <p className="mt-2 border-t border-[var(--line)] pt-2 text-[11px] leading-snug text-[var(--text-3)]">
                {adapterDescription}
          ...
          <Panel title="Endpoints">
            <div className="px-3 py-2">
              ...
              <KeyValue label="Sources registered" dense>
                <Num
                  value={Object.keys(provenance.data?.sources ?? {}).length}
                  digits={0}
                />
              </KeyValue>
